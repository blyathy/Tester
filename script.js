...
const questions = [
  ...
  function startTest() {
    ...
    if (count === 20) {
      selectedQuestions = questions.slice().sort(() => 0.5 - Math.random()).slice(0, 20);
    } else {
  ...
  function showQuestion() {
    ...
    document.getElementById("questionBox").textContent = `${currentQuestion + 1}. ${q.question}`;
  
    // 1. Скопировать и перемешать варианты
    const shuffledOptions = q.options.slice().sort(() => Math.random() - 0.5);
  
    // 2. Отрисовать кнопки из перемешанного массива
  ...
  function showResult() {
    ...
    resultBox.textContent = `Вы набрали ${score} из ${questionLimit} правильных ответов.`;
    resultBox.classList.remove("hidden");
  
    const mistakes = userAnswers.filter(a => a.selected !== a.correct);
    if (mistakes.length) {
      const errorsDiv = document.getElementById("errors");
      errorsDiv.innerHTML = "<h2>Ошибки:</h2><ul id=\"errorList\"></ul>";
      const ul = document.getElementById("errorList");
      mistakes.forEach(item => {
        const li = document.createElement("li");
        li.textContent = `${item.question} — вы выбрали "${item.selected}", правильно: "${item.correct}"`;
        ul.appendChild(li);
      });
